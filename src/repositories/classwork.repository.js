...
async function getTopicsUpToSession(sessionId) {
  const session = await db('class_session_plans').where({ id: sessionId }).first();
  ...
  let cumulativeTopicIds = [...new Set(previousSessions.map((s) => s.topic_id))];
...
async function getPresentStudentsBySession(sessionId) {
  ...
  const attendanceSession = await db('attendance_sessions')
    .where({ class_id: session.class_id, session_date: session.session_date, is_active: true })
    .first();
...
async function upsertSheet({ sessionId, topicId, subtopicId, description, entries = [], userId }) {
